...
exports.handler = async (event) => {
  ...
  try {
    ...
    console.log('Starting email sends. WEB3FORMS_KEY present:', !!WEB3FORMS_KEY, '| STUDIO_EMAIL:', STUDIO_EMAIL);
...
🌐 lizwendybeautystudiollc.com
  ...
  const res = await fetch('https://api.web3forms.com/submit', {
    ...
    headers: {
      ...
      'Origin': 'https://lizwendybeautystudiollc.com',
      'Referer': 'https://lizwendybeautystudiollc.com/book.html',
    },
    body: JSON.stringify({
  ...
  console.log('sendCustomerEmail → status:', res.status, '| response:', JSON.stringify(json));
...
This appointment has been automatically added to the Liz Wendy Bookings calendar.
  ...
  console.log('sendStudioEmail → sending to:', STUDIO_EMAIL);
  ...
  const json = await res.json().catch(() => ({}));
  console.log('sendStudioEmail → status:', res.status, '| response:', JSON.stringify(json));
  return { ok: res.ok, status: res.status, response: json };
}
